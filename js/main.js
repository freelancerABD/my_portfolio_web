...
(()=>{
	...
	document.addEventListener("click", (event) =>{
		if (event.target.classList.contains('link-item')){
			/* make sure event.target.hash has a value before overridding defult
			behavior */
			if (event.target.hash !=="") {
				// prevent defult anchor click behavior
				event.preventDefault();
				const hash = event.target.hash;
				// deactivate existing anchor 'section'
...
(() =>{
	...
	function popupSlideshow(){
		...
		popupImg.src= imgSrc;
		popupImg.onload = () =>{
		  // deactivete loader after the popupImg loded
	 ...
	 function popupDetails(){
	 	...
	 	const details = portfolioItems[itemIndex].querySelector(".info").innerHTML;
	 	//set the project details
	 	popup.querySelector(".info").innerHTML = details;
